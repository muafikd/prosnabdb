...
const props = defineProps<{ visible: boolean, proposalId: number | null }>()
const emit = defineEmits(['update:visible', 'saved'])
const dialogVisible = computed({
  get: () => props.visible,
  set: (val) => emit('update:visible', val)
})
const loadProposalData = async (proposalId: number) => {
  await handleEdit({ proposal_id: proposalId } as any)
}
...
watch(() => props.visible, async (newVal) => {
  if (newVal) {
    if (props.proposalId) {
      ...
      await loadProposalData(props.proposalId)
    } else {
